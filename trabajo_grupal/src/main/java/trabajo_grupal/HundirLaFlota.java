...
import trabajo_grupal.util.Scoreboard;
...
for (int i = 0; i < tablero.length; i++) {
    ...
    private boolean verificarDerrota(Tablero logicaTablero, String usernameGanador) {
        for (Barco barco : logicaTablero.getBarcos()) {
            if (!barco.estaHundido()) {
                return false; // Aún queda un barco, el jugador no ha perdido
            }
        }

        JOptionPane.showMessageDialog(frame, "🎉 ¡" + usernameGanador + " ha ganado!");
        Scoreboard.actualizarVictorias(usernameGanador); // Registrar victoria en la base de datos
        mostrarClasificacion(usernameGanador); // Mostrar el ranking actualizado
        return true;
    }
...
private void manejarDisparo(JButton boton, int fila, int columna, Tablero logicaTablero, JButton[][] tableroBotones) {
    ...
    if (verificarDerrota(logicaTablero, null)) {
	...
	private void mostrarClasificacion(String username) {
		SwingUtilities.invokeLater(() -> new Scoreboard(username));
}
